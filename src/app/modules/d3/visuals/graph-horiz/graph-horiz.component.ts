...
interface FlareNode {
    name: string;
    children?: Array<FlareNode>
}

@Component({
    selector: "graph-horiz",
    templateUrl: './graph-horiz.component.html',
    styleUrls: ["graph-horiz.component.scss"],
    providers: [],
    encapsulation: ViewEncapsulation.None,
})
export class GraphHorizComponent implements OnInit {
    private svg;
    private treeLayout;
    private root;
    ...
    ngOnInit() {
        d3.json("../../../../../assets/flare.json").then(data => {
            data = data as Array<FlareNode>;
            let test = data as FlareNode;
            // this.root = d3.hierarchy(data, (d) => d.children);
            this.root = d3.hierarchy(test, (d) => d.children);
